        ...
        <template v-if="task.id">
            <template v-if="task.type==3">
                ...
                <div class="content">
                    <div class="profile-head pb-2 row">
                        <div class="col-md-3">
                            <a href="#">
                                <img class="profile-m-2 mr-1 float-left"
                                     v-bind:src="task.assigner.image">
                                <h6 class="white-label">{{ lang('Assigner')
                                    }}</h6>
                                <h3 class="mb-1 text-white">{{
                                    task.assigner.name }}</h3>
                            </a>
                        </div>
                        <div class="col-md-3">
                            <a href="#">
                                <img class="profile-m-2 mr-1 float-left"
                                     v-bind:src="task.assignee.image">
                                <h6 class="white-label">{{ lang('Assign to')
                                    }}</h6>
                                <h3 class="mb-1 text-white">{{
                                    task.assignee.name }}</h3>
                            </a>
                        </div>
                        <div class="col-md-3">
                            <h6 class="white-label">{{ lang('Task Sent') }}</h6>
                            <h3 class="mb-1 text-white">{{ task.created_at |
                                moment("DD.MM.") }}</h3>
                        </div>
                        <div class="col-md-3">
                        </div>
                    ...
                    <div class="showdata-box row">
                        <div class="col-md-9">
                            ...
                            <div>
                                <h4 class="mb-1">
                                    {{ task.assigner.name }} has requested an
                                    expense approval for {{ task.related.name }}
                                    expense.<br>
                                    {{ task.description }}<br>
                                </h4>
                                <a class="btn btn-neutral btn-addon mb-4 mr-5"
                                   v-bind:href="task.related_link"
                                   v-if="task.related_link">{{ lang('Go To Project') }}</a>
                            </div>
                            <table class="table">
                                <thead class="thead-inverse">
                                <tr>
                                    <th>{{ lang('Item') }}</th>
                                    <th class="text-right">{{ lang('Budget')
                                        }}
                                    </th>
                                    <th class="text-right">{{ lang('Expense Total') }}
                                    </th>
                                    <th class="text-right">{{ lang('Difference')
                                        }}
                                    </th>
                                </tr>
                                </thead>
                                <tbody class="white">
                                <tr>
                                    <td v-bind:data-title="lang('Title')">{{ lang(task.related.name) }}
                                    </td>
                                    <td class="text-right"
                                        v-bind:data-title="lang('Budget')">{{ task.related.budget | flexCurrency('kn', 2) }}
                                    </td>
                                    <td class="text-right"
                                        v-bind:data-title="lang('Expense Total')">
                                        {{ task.related.expense | flexCurrency(' kn', 2) }}
                                    </td>
                                    <td class="text-right"
                                        v-bind:data-title="lang('Difference')">
                                        {{ Math.abs(task.related.budget -
                                        task.related.expense) | flexCurrency(' kn', 2) }}
                                    </td>
                                </tr>
                                </tbody>
                        ...
                        <div class="col-md-3">
                            <div class="page-name-l mt-2 mb-1">{{ lang('Task Type') }}
                            </div>
                            <div>
                                <h4 v-bind:class="['mb-1', task_types[task.type].className]">
                                    {{ lang(task_types[task.type].title) }}</h4>
                            </div>
                        </div>
                    </div>

                    <div class="comments">
                        <div class="page-name-xl mb-3">{{ lang('Comments') }}
                        </div>
                        <!-- Textarea -->
                        <div class="content">
                        ...
                        <div class="justify-content-end d-flex mb-2">
                            <spinner-button classes="btn btn-neutral"
                                            v-on:button_clicked="commentTask"
                                            title="Send"></spinner-button>
                        </div>
                        <template v-if="task.thread">
                            <div v-for="message in filteredMessages">
                                <h3 class="page-name-l mb-1"><span>{{ message.sender.name }},</span>
                                    {{ message.created_at | moment('DD.MM.YYYY., H:mm:ss') }}</h3>
            ...
            <template v-else-if="task.type==5">
                ...
                <div class="content">
                    ...
                    <div class="showdata-box row">
                        <div class="col-md-9">
                            <div class="page-name-l mt-2 mb-1">{{ lang('Task Description') }}
                            </div>
                            <div>
                                <h4 class="mb-1">
            ...
            <template v-else>
                ...
                <div class="content">
                    <div class="profile-head pb-2 row">
                        ...
                        <div class="col-md-3">
                            <h6 class="white-label">{{ lang('Deadline / Priority') }}<span
                                        class="badge badge-danger display-d mt-1 float-right">{{ lang(priorities[task.priority]) }}</span>
                            </h6>
                    ...
                    <!-- Stopwatch -->
                    <div class="page-name-xl mt-2 mb-4">{{ lang('Working Time') }}
                    </div>
                    <div class="stopwatch mt-3">
                        ...
                        <div class="stopwatch-controls">
                            <div class="d-flex justify-content-center mt-2">
                                <button id="play"
                                        class="btn btn-lg btn-success btn-start-icon"
                                        v-on:click="startClock">{{ lang('Play') }}
                                </button>
                                <button id="stop"
                                        class="btn btn-lg btn-danger btn-stop-icon"
                                        v-on:click="stopClock">{{ lang('Stop') }}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Initial Documents -->
                    <div class="page-name-xl mt-4">{{ lang('Initial Documents') }}
                    </div>
                    <div class="files mt-2 mb-2">
                        <div class="file-box file-box-l d-flex align-items-center"
                             v-for="(document,index) in task.files.initial">
                            <a v-bind:href="document.link"
                               v-on:click.prevent="documentDownload(document.link)"
                               class="file-icon">{{ document.title }}</a>
                            <div class="file-box-sty ml-auto d-flex">
                                <a v-bind:href="'/human_resources/employee/'+document.owner.id+'/show'"><img
                                        class="profile-m-1 mr-1 align-self-center"
                                        v-bind:src="document.owner.image">
                                    {{ document.owner.name }}
                                </a></div>
                            <div class="file-box-sty">{{ document.date }}</div>
                            <div class="file-box-sty icon icon-download"
                                 v-on:click="documentDownload(document.link)">{{ lang('Download') }}
                            </div>
                            <div class="file-box-sty icon icon-cancel"
                                 v-on:click="fileWarning(index, 'initial')">{{ lang('Delete') }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="justify-content-center d-flex mt-2 mb-4">
                        <button type="button" class="btn btn-neutral"
                                v-on:click="documentAdd('initial-documents')">{{ lang('Upload') }}
                        </button>
                    </div>

                    <!-- Final Documents -->
                    <div class="page-name-xl mt-4">{{ lang('Final Documents') }}
                    </div>
                    <div class="files mt-2 mb-2">
                        <div class="file-box file-box-l d-flex align-items-center"
                             v-for="(document,index) in task.files.final">
                            <a v-bind:href="document.link"
                               v-on:click.prevent="documentDownload(document.link)"
                               class="file-icon">{{ document.title }}</a>
                            <div class="file-box-sty ml-auto d-flex">
                            ...
                            <div class="file-box-sty icon icon-cancel"
                                 v-on:click="fileWarning(index, 'final')">{{ lang('Delete') }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="justify-content-center d-flex mt-2 mb-4">
                        <button type="button" class="btn btn-neutral"
                                v-on:click="documentAdd('final-documents')">{{ lang('Upload') }}
                        </button>
                    </div>
                    ...
                    <!-- Footer buttons -->
                    <div class="btn-footer mt-2 mb-2 flex-column flex-md-row d-flex p-2">
                        <button v-if="canAccept()" class="btn btn-lg btn-save"
                                v-on:click="acceptTask">{{ lang('Accept') }}
                        </button>
                        <button v-if="canAssign()"
                                class="btn btn-lg btn-assign btn-assign-icon"
                                v-on:click="openModal('modal-reassign')">{{ lang('Assign to...') }}
                        </button>
                        <button v-if="canComplete()"
                                class="btn btn-lg btn-save btn-complete-icon"
                                v-on:click="completeTask">{{ lang('Complete') }}
                        </button>
                        <button v-if="canResend()" class="btn btn-lg btn-save"
                                v-on:click="resendTask">{{ lang('Resend') }}
                        </button>
                        <button v-if="canDelete()" class="btn btn-lg btn-cancel"
                                v-on:click="taskWarning">{{ lang('Delete') }}
                        </button>
                    </div>
                </div>
                ...
                <warning-modal v-on:warningConfirmed="listenForWarning"
                               v-on:warningCanceled="clearDelete"></warning-modal>
            </template>
        </template>
        <template v-else>
        ...
        methods: {
            ...
            approveRequest() {
                axios.post('/api/task/' + this.task.id + '/accept')
                    .then(() => {
                        this.$eventHub.emit('BUTTON_LISTEN_FOR_SUCCESS');
                        toastr.success(this.lang('Successfully approved'));
                        this.$router.push('/tasks');
                    }).catch(() => {
                    this.$eventHub.emit('BUTTON_LISTEN_FOR_FAILURE');
            ...
            rejectRequest() {
                axios.post('/api/task/' + this.task.id + '/reject')
                    .then(() => {
                        this.$eventHub.emit('BUTTON_LISTEN_FOR_SUCCESS');
                        toastr.success(this.lang('Successfully rejected'));
                        this.$router.push('/tasks')
                    }).catch(() => {
                    this.$eventHub.emit('BUTTON_LISTEN_FOR_FAILURE');
                    toastr.error(this.lang('There was an error. Please try again'));
        ...
        mounted: function () {
            ...
            axios.get('/api/task/' + id)
                .then((res) => {
                    ...
                    Echo.private('thread.' + this.task.thread.id)
                        .notification((comment) => {
                            this.task.thread.messages.push(comment.data)
                        });
                })
                .catch((err) => {
                });
