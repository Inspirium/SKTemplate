<template>
    <div class="content">
        <div class="page-name-xl mb-2 mt-3">{{ lang( 'Departments' ) }}</div>
        ...
        <div class="btn-header d-flex p-2">
            <router-link to="/human_resources/department/new" class="btn btn-lg btn-blank"><i class="fa fa-plus left"></i>{{ lang( 'Add New Department' ) }}</router-link>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            queryHandler() {
                axios.post('/api/human_resources/departments', this.query)
                    .then((res) => {
